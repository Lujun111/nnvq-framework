...
if [ $stage -le -1 ]; then
    ...
    for dataset in train test dev; do
        ...
        if [[ "$dataset" == "train" ]]; then
            python $framework_path/KaldiHelper/KaldiMiscHelper.py --nj $nj $data $working_dir/alignments/all_ali_${string}_${dataset} $working_dir/tmp_${dataset} || exit 1;
        else
            python $framework_path/KaldiHelper/KaldiMiscHelper.py --nj $njd $dataset $working_dir/alignments/all_ali_${string}_${dataset} $working_dir/tmp_${dataset} || exit 1;
        fi
    ...
    for dataset in train test dev; do
        if [[ "$dataset" == "train" ]]; then
            python $framework_path/KaldiHelper/MiscHelper.py --nj $nj --state-based $state_based $framework_path/stats.mat \
                $working_dir/tmp_${dataset} $working_dir/tf_data/${dataset} || exit 1;
        else
