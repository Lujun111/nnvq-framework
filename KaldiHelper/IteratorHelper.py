...
class DataIterator(object):
    def __init__(self, nj, folder):
        self._path = '../kaldi/egs/tedlium/s5_r2'
        self._nj = nj
        self._folder = folder
    ...
    def _create_iterator(self):
        ...
        if ('/' or '..') not in self._folder:
            print(self._path + '/data/' + self._folder)
            assert (os.path.isdir(self._path + '/data/' + self._folder))
            self._generator = ('add-deltas scp:' + self._path + '/data/' + self._folder + '/split' + str(self._nj) + '/' + str(i) +
