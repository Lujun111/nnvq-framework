...
train_writer = tf.summary.FileWriter('logs/4184lstm4/train', sess.graph, flush_secs=600)
dev_writer = tf.summary.FileWriter('logs/4184lstm4/dev', sess.graph, flush_secs=600)
                ...
                if num_input==num_label:
                    for k in range(num_input):
                        ...
                        if train_accuracy > max_acc_train:
                            ...
                            saver.save(sess, "models/4184lstm4/train/model.ckpt",global_step=step)
                ...
                if num_input == num_label:
                    for k in range(num_input):
                        ...
                        if dev_accuracy > max_acc_dev:
                            ...
                            saver.save(sess,"models/4184lstm4/dev/model.ckpt",global_step=step_dev)
...
nj = 30
path = '/home/ge69yif/kaldi/egs/tedlium/s5_r2/data/test_hires/split30'
counts = load_counts('4184_pdf.counts')
saver.restore(sess, tf.train.latest_checkpoint('models/4184lstm4/dev/'))
for i in range(1,nj+1):
    out_file = 'ark:| copy-feats --compress=true ark:- ark:'+path+'/'+str(i)+'/post.ark'
    post_file = kaldi_io.open_or_fd(out_file, 'wb')

    fea = {k: m for k, m in kaldi_io.read_mat_scp(path+'/'+str(i)+'/feats_cmvn.scp')}
