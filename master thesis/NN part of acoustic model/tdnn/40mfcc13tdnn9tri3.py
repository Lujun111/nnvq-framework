...
output = tf.layers.dense(BN4,units=pdf_nums,name='fully-connected')
...
train_writer = tf.summary.FileWriter('logs/4184tdnn6_2/train', sess.graph,flush_secs=600)
dev_writer = tf.summary.FileWriter('logs/4184tdnn6_2/dev', sess.graph,flush_secs=600)
...
for i in range(1,num_epochs+1):
    ...
    for j in range(1, nj_train_end+1):
        features_train_path = '/home/ge69yif/kaldi/egs/tedlium/s5_r2/data/train_cleaned_sp_hires_comb/split30/'+str(j)+'/feats_cmvn.scp'
        pdfs_train_path = '/home/ge69yif/kaldi/egs/tedlium/s5_r2/exp/tri3_cleaned_ali_train_cleaned_sp_comb/dataset/pdf.'+str(j)+'.ark'
        ...
        for key in fea_train_keys:
            if key in pdf_train_reader.keys():
                ...
                if train_accuracy > max_acc_train:
                    ...
                    saver.save(sess,"models/4184tdnn6_2/train/model.ckpt", global_step=step)
    ...
    for j in range(nj_train_end+1, nj_all+1):
        features_dev_path = '/home/ge69yif/kaldi/egs/tedlium/s5_r2/data/train_cleaned_sp_hires_comb/split30/'+str(j)+'/feats_cmvn.scp'
        pdfs_dev_path = '/home/ge69yif/kaldi/egs/tedlium/s5_r2/exp/tri3_cleaned_ali_train_cleaned_sp_comb/dataset/pdf.'+str(j)+'.ark'
        ...
        for key in fea_dev_keys:
            if key in pdf_dev_reader.keys():
                ...
                if dev_accuracy > max_acc_dev:
                    ...
                    saver.save(sess,"models/4184tdnn6_2/dev/model.ckpt",global_step=step_dev)
...
nj = 30
test_path = '/home/ge69yif/kaldi/egs/tedlium/s5_r2/data/test_hires/split30'
counts = load_counts('4184_pdf.counts')
saver.restore(sess, tf.train.latest_checkpoint('models/4184tdnn6_2/dev/'))
for i in range(1,nj+1):
    out_file = 'ark:| copy-feats --compress=true ark:- ark:'+test_path+'/'+str(i)+'/post.ark'
    post_file = kaldi_io.open_or_fd(out_file, 'wb')

    fea = {k: m for k, m in kaldi_io.read_mat_scp(test_path+'/'+str(i)+'/feats_cmvn.scp')}
