...
output = tf.layers.dense(BN4,units=4152,name='fully-connected')
...
with tf.name_scope("loss"):
    onehot_labels = tf.one_hot(tf.cast(labels, tf.int32), 4152, axis=-1)
...
train_writer = tf.summary.FileWriter('logs/4152tdnn12sgd/train', sess.graph)
dev_writer = tf.summary.FileWriter('logs/4152tdnn12sgd/dev', sess.graph)
        ...
        for key in fea_train_keys:
            if key in pdf_train_reader.keys():
                ...
                if train_accuracy > max_acc_train:
                    ...
                    saver.save(sess,"models/4152tdnn12sgd/train/model.ckpt", global_step=step+1)
        ...
        for key in fea_dev_keys:
            if key in pdf_dev_reader.keys():
                ...
                if dev_accuracy > max_acc_dev:
                    ...
                    saver.save(sess,"models/4152tdnn12sgd/dev/model.ckpt")
...
saver.restore(sess, tf.train.latest_checkpoint('models/4152tdnn12sgd/dev/'))
